...
export type TshirtBreakpointBounds = [TshirtBreakpointSizeAny, TshirtBreakpointSizeAny];

export type TshirtBreakpointValue = number | string;

export type TshirtBreakpointRange<TBreakpoints extends TshirtBreakpoints> = TshirtRange<TshirtBreakpointsToSizeTuple<TBreakpoints>>;
...
const parseBreakpointValue = (value: TshirtBreakpointValue) => typeof value === 'number' ? `${value}px` : value;

const breakpointValueToQuery = (value: TshirtBreakpointValue, type: 'min' | 'max') => `(${type}-width: ${parseBreakpointValue(value)})`;
...
export type TshirtBreakpoints<TBounds extends TshirtBreakpointBounds = never> =
  [TBounds] extends [never] ? TshirtRecord<typeof ALL_BREAKPOINTS_BOUNDS, TshirtBreakpointValue> :
...
export type TshirtBreakpointCallback<TBreakpoints extends TshirtBreakpoints = never> =
  (size: TshirtBreakpointsToSize<TBreakpoints>, value: TshirtBreakpointValue) => undefined | string;

export const tshirtBreakpointRangeToSizes = <TBreakpoints extends TshirtBreakpoints>(breakpoints: TBreakpoints, ...ranges: ReadonlyArray<TshirtBreakpointRange<TBreakpoints>>) =>
...
export const mapTshirtBreakpointsToCss
  = <TBreakpoints extends TshirtBreakpoints>(breakpoints: TBreakpoints, type: 'min' | 'max', callback: TshirtBreakpointCallback<TBreakpoints>) =>
    Object.entries(breakpoints).flatMap(([size, breakpoint]) => {
      ...
      const value = (callback as TshirtBreakpointCallback)(size as TshirtBreakpointSizeAny, breakpoint as TshirtBreakpointSizeAny);
      if (!value) {
        return [];
      }
...
export const tshirtBreakpointsToCssVariables = <TBreakpoints extends TshirtBreakpoints>(breakpoints: TBreakpoints, prefix: string) =>
  Object.fromEntries(['min', ...Object.keys(breakpoints)].map(breakpointSize => [breakpointSize, createCssVariable(breakpointSize as TshirtBreakpointSizeAny, prefix)])) as Record<'min' | keyof TshirtBreakpoints, `--${string}`>;
...
export const tshirtBreakpointsToCss = (breakpoints: TshirtBreakpoints, prefix: string) => {
  ...
  return toValue('min') + mapTshirtBreakpointsToCss(breakpoints, 'min', size =>
    toValue(size),
  );
};

export const tshirtBreakpointRangeToInline = <TBreakpoints extends TshirtBreakpoints>(breakpoints: TBreakpoints, prefix: string, range: TshirtBreakpointRange<TBreakpoints>) => {
