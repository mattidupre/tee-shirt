...
> = TEnd extends TStart ? TResult | TStart : _IntegerRange<Add<TStart, 1>, TEnd, TResult | TStart>;
...
> = TArray extends readonly number[] ?
  TEnd extends TStart ? [...TArray, TStart] :
...
export type IntegerRangeTuple<
  TStart extends number,
  TEnd extends number,
> = _IsValidRange<TStart, TEnd> extends 1 ? _IntegerRangeTuple<TStart, TEnd> : never;

/**
 * Constructs an array of mapped values from integers within the provided range.
 */
export const mapIntegerRange = <T>(start: number, end: number, callback: (number: number) => T): readonly T[] => {
  const result: T[] = [];
